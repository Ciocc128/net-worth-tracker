...
## Chiusura: cancellazione delle spec e riallineamento dei prompt — 2026-07-28

**Spec cancellate** (`docs/specs/{6,7,8,9,10}-*.md`). Tutte implementate e verificate sui dati reali, quindi la fonte di verità torna a essere codice + AGENTS.md + CLAUDE.md. Recuperabili dalla git history: `git show 55819e3^:docs/specs/<file>.md`. Stessa decisione del 2026-07-27 per le spec precedenti — niente file ARCHIVE.md.

Ripuliti **tutti** i riferimenti, riformulando ogni frase perché stia in piedi da sola invece di puntare a un file inesistente: `AssetDialog.tsx`, `settings/page.tsx` (3 siti), `performanceService.ts` (cronologia di `CACHE_MATH_VERSION`), 4 file di test, CLAUDE.md (5 siti) e AGENTS.md. Nell'occasione tolti anche **5 riferimenti alle spec cancellate a luglio** (`spec 01 §5`, `spec 02 §4`, `spec 04 §3`, `spec 6-…`) sopravvissuti al primo giro di pulizia: usavano la notazione "spec NN §M" invece del percorso del file, quindi la grep di allora non li aveva presi. Verifica finale: nessuna occorrenza di `docs/specs` o `spec NN` in tutto il repo fuori da questo file.

**Prompt riallineati** (`docs/{critique,audit}-prompts.md`, sezione Rendimenti — l'unica pagina toccata dalla sessione; le altre ~19 sezioni sono ancora corrette):
- **Errore preesistente**: il period selector era descritto come `1M/3M/YTD/1Y/3Y/5Y/ALL`, ma 1M e 3M non esistono. I periodi reali sono YTD / 1 Anno / 3 Anni / 5 Anni / Storico + Personalizzato (`periodLabels`, `page.tsx:314`).
- Pure layer: aggiunti `drawdownSeries.ts` e `cashFlowMap.ts`; componenti: aggiunto `PerformanceTooltip`.
- Base delle metriche: da "i fondi pensione sono esclusi" a "configurabile dalle Impostazioni, dichiarata sotto l'hero".
- Aggiunti il grafico Evoluzione (area + linea, **mai** bande impilate, col perché) e i comportamenti di onestà del 2026-07-28 (rendimento di periodo sotto i 6 mesi, "—" sotto 3 osservazioni, "dal massimo del periodo", riconciliazione heatmap/Underwater/MaxDD).
- Audit: l'avviso amber di `RealizedGainsSection` aggiunto alle eccezioni **Amber Watch** (era già convenzione per `PerformanceHero`, ora c'è un secondo sito e senza la nota un audit lo segnalerebbe come violazione); più un avviso sui due omonimi — la card **"Capitale investito"** (registro operazioni: acquisti − vendite) e l'area **"Capitale immesso"** (grafico: patrimonio iniziale + versamenti da Cashflow) — con l'istruzione esplicita di NON uniformare i nomi.

**Gate**: `npx tsc --noEmit` pulito. `npx vitest run` **79 file / 1406 test**. `npm run build` ok.
