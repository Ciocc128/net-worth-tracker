...
- **The Accumulo tile (PAC)** is a standalone feature beside Piano — its own plan document, its own rules, its own guide: `doc/guide/accumulo.md`.
- **Composizione ideale** (`ComposizioneIdealeTile`, right after Accumulo, `desktop:col-span-12`, G4) is the weight optimizer's SECOND entry point (the PAC's Ottimizzato view is the first) — «com'è fatto il mio portafoglio ideale, strumento per strumento?», with no plan required: candidates are the tradable assets directly, one per instrument, no proxy grouping. Off when `idealAllocation` is disabled (a link to Impostazioni, same as the PAC's OptimizerPanel); on, opens `IdealCompositionDialog` on click, no calculation before that. Never writes on its own; "Crea un PAC con questi pesi" hands the proposed weights to a brand-new `AccumulationPlanDialog` (never a second modal stacked on the first — the tile owns which of the two is mounted), disabled with a reason when a draft/active plan already exists. doc/guide/ottimizzatore.md § Due punti d'ingresso.
