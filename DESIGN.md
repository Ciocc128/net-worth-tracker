...
`variant="legacy"` is the pre-redesign header (2xl/3xl title, description under it, optional `border-b`), declared explicitly by the pages not yet propagated (Analisi, Rendimenti, Storico, Allocazione, Previdenza) so their render is byte-identical; it is deleted with the last of them.
...
tile-level form of the rule Centri di Costo states for a view: *a surface that shows a period must
be able to change it, or must name the window of every figure that uses a different one.* Never
"fix" it by wiring the axis into a figure the axis cannot honestly move.
