...
`variant="legacy"` is the pre-redesign header (2xl/3xl title, description under it, optional `border-b`), declared explicitly by the page not yet propagated (Previdenza) so its render is byte-identical; it is deleted with it.
...
Used inside collapsible sections where slight extra separation aids readability of many parameters side by side — the scenario parameters of FIRE › Calcolatore's «Parametri» disclosure keep it (2026-08-25): three bordered tiles, one per scenario, inside the Scenari tile of the disclosure.
