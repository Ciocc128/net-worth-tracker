...
`variant="legacy"` is the pre-redesign header (2xl/3xl title, description under it, optional `border-b`), declared explicitly by the pages not yet propagated (Storico, Allocazione, Previdenza) so their render is byte-identical; it is deleted with the last of them.
