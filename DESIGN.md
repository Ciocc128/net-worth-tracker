...
colors:
  ...
  # Semantic — sign colours. These are the DEFAULT theme's LIGHT values; the six named themes
...
rounded:
  ...
  # xl = r+4). The values below are the default theme's (r = 0.625rem = 10px); the six named
  # themes set r to 0.25 / 0.5 / 0.3 / 0.375 / 0.5 / 1rem, so on Retro Arcade sm–xl are 0/2/4/8.
...
The six named color themes (Solar Dusk, Elegant Luxury, Midnight Bloom, Cyberpunk, Retro Arcade, Lime Frost) are personality layers on top of a structural foundation. They change accent and surface palette — and, since each declares its own `--radius`, the derived `sm`–`xl` corners (`rounded-md` is 8px on the default theme and 2px on Retro Arcade) — without touching the type scale, the component API, or the tile's fixed 16px `2xl` (corrected 2026-09-06; see the `rounded` comment in the frontmatter). The default theme is the instrument in its raw state. The themes are its finishes.
...
- **Destructive Flame** (`oklch(0.577 0.245 27.325)`): Destructive actions **and** the negative half of every sign colour. Saturated enough to demand attention without being an emergency siren. The named themes each declare their own; seven of the twelve declarations of that date were raised or lowered on 2026-08-13 to clear 4.5:1 as text against their own `--card` — the default theme already did and was left untouched.
- **Positive Jade** (`oklch(0.482 0.194 149.214)` light / `oklch(0.740 0.156 148.655)` dark): the positive half of every sign colour. **One named theme overrides it: Lime Frost light** (`oklch(0.50 0.15 138)`, 2026-09-14, so gains sit in the same green family as its income series; its dark block takes the default back) — every other combination is served by this one pair. The light value was darkened from `0.627` on 2026-08-13: at that lightness it measured 2.62–3.22:1 across the light themes, so the reassuring half of every verdict read fainter than the alarming half — an asymmetry in what the interface was emphasising, not just a contrast number.
...
**The Data Owns Color Rule.** Chart palettes, performance indicators, and the six named themes are the only sanctioned sources of chromatic energy. Interface chrome in the default theme is always achromatic.
...
- **Do** let the six named themes handle personality. Resist adding theme-like color to the default palette.
