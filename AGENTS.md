...
- A coupon's cashflow expense is created only by the daily cron on payment date (`!isAutoGenerated`, idempotent via `expenseId`). A scraped dividend has ONE floor (`lib/utils/dividendEligibility.ts`, 2026-09-13), never silent. Adding a `DividendType` is a six-file fan-out.
