...
const SAVINGS_COLOR = 'var(--flow-in)';
const MARKET_COLOR = 'var(--hero-series)';
/** A losing market month: the theme's chart loss, softer than the text red on a filled bar. */
const MARKET_LOSS_COLOR = 'var(--sign-chart-loss)';
...
const BAR_LEGEND: readonly SeriesLegendItem[] = [
  ...
  { label: 'Mercato (rosso se in perdita)', colors: [MARKET_COLOR, MARKET_LOSS_COLOR] },
 ...
 * fit a four-column tile. The current month sits on the app's current-slot band, never the others dimmed.
 *
          ...
          {months.map((m, i) => {
            ...
            return (
              <g key={`${m.year}-${m.month}`}>
                ...
                {bar(m.market, x0 + barWidth + gap, MARKET_COLOR, MARKET_LOSS_COLOR)}
...
export function DriverTile({ reading, years, featured, total, startYear, measuredSince, months, windowMonths, scrubIndex = null, className }: DriverTileProps) {
        ...
        <>
          ...
          {months.length > 0 && (
            <>
              ...
              <DriverBars months={months} scrubIndex={scrubIndex} className="mt-2.5 flex-1" />
            </>
          )}
        </>
      )}
