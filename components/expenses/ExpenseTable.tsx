  ...
  return (
    <TableRow className={cn(armed && 'bg-destructive/5')}>
        ...
        <div className="flex items-center gap-2">
          {CatIcon ? (
            <div
              ...
              style={{ backgroundColor: categoryIconBackground(categoryMeta?.color) }}
            >
              <Suspense fallback={null}>
                <CatIcon className="h-3 w-3" style={{ color: categoryIconColor(categoryMeta?.color) }} aria-hidden="true" />
              </Suspense>
            </div>
          ) : categoryMeta?.color ? (
            <div className="h-2.5 w-2.5 flex-shrink-0 rounded-full" style={{ backgroundColor: categoryIconColor(categoryMeta.color) }} />
          ) : null}
      ...
      <TableCell className={cn('text-right', FIGURE_CLASS)}>
        {/* The figure tokens (--income-figure / --expense-figure) carry the theme's rule on flows; a scheduled row is muted. */}
        <div
          className={cn(
            'flex items-center justify-end gap-1 font-medium',
            scheduled ? 'text-muted-foreground' : expense.type === 'income' ? 'text-income-figure' : 'text-expense-figure',
          )}
        >
