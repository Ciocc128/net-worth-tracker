...
- **11 anonymized product screenshots** in `docs/screenshots/` — six are of the new generation: `portfolio-overview.png` (retaken 2026-08-22), `dividend-calendar.png` (retaken 2026-08-23), `cashflow-sankey.png` and `cashflow-drilldown.png` (both retaken 2026-08-25 on Analisi: the Flusso tile with its subcategory layer, and the Scheda of a subcategory), `performance-metrics.png` and `monthly-heatmap.png` (both retaken 2026-08-25 on Rendimenti: the verdict over the first two rows of tiles on «1 anno», and the Consistenza tile with the heatmap in the sign tokens), all from throwaway synthetic emulator accounts, after the shell redesign and the «Verdict over Tiles» propagation: compact header, sidebar with eyebrow group labels and the assistant as a route, tiles. The other five date from 2026-06-07, show the previous generation including the old sidebar, and none covers Previdenza, Assistente, Storico per-instrument, Goal-Based Investing or Impostazioni.
