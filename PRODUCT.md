...
- **11 anonymized product screenshots** in `docs/screenshots/` — two are of the new generation: `portfolio-overview.png` (retaken 2026-08-22) and `dividend-calendar.png` (retaken 2026-08-23, both from throwaway synthetic emulator accounts, after the shell redesign and the «Verdict over Tiles» propagation: compact header, sidebar with eyebrow group labels and the assistant as a route, tiles). The other nine date from 2026-06-07, show the previous generation including the old sidebar, and none covers Previdenza, Analisi, Assistente, Storico per-instrument, Goal-Based Investing or Impostazioni.
