 ...
 *
 * The clause of the view the reader is IN comes first (upstream, 2026-09-21). The tile shows one
 * list at a time, and a reading that opened on the heaviest holding while the list ranked issuers
 * answered a question nobody had asked. The other facts stay, as the context of the first one;
 * Geografia has no clause of its own, so it keeps the holding-first order.
 */
export function describeExposure(highlights: ExposureHighlights, view: ExposureViewKey = 'holdings'): Narrative | null {
  ...
  const holdingClause = (): Clause | null => {
    ...
    return {
      narrative: [prose(`il titolo più pesante è ${name} (`), percent(pct, 1), prose(` dell'azionario, in ${sourceCount} strument${sourceCount === 1 ? 'o' : 'i'})`)],
  ...
  const issuerClause = (): Clause | null => {
    ...
    return {
      narrative: [prose(`${family} gestisce ${articleForPercent(pct, 0)}`), figure(`${Math.round(pct)}%`), prose(' del portafoglio')],
      opensOnProperName: true,
    };
  };
  const currencyClause = (): Clause | null => {
    if (!highlights.currencyQuotationContrast || !highlights.topCurrency) return null;
    const { label, pct } = highlights.topCurrency;
    return {
      narrative: [prose(`gran parte del rischio valutario è in ${label} (`), percent(pct, 0), prose('), anche se ogni tuo strumento quota in euro')],
      opensOnProperName: false,
    };
  };
  ...
  const order: Array<Clause | null> =
    view === 'sectors'
      ? [sectorClause(), holdingClause(), issuerClause(), currencyClause()]
      : view === 'issuers'
        ? [issuerClause(), holdingClause(), sectorClause(), currencyClause()]
        : view === 'currency'
          ? [currencyClause(), holdingClause(), sectorClause(), issuerClause()]
          : [holdingClause(), sectorClause(), issuerClause(), currencyClause()];
