...
- **The type of a saved entry can be changed** across the four single-account types (Variable, Fixed, Debt, Income): the category re-attaches itself to the same-named one under the new type, the amount's sign and the linked account balance are corrected when you cross the income boundary, and an inline note states what will change before you save. Transfers stay locked, since they involve two accounts
- **Categories are identified by document, not by name**, so the same name can live under two types (a "Casa" under Fixed and another under Variable) without their figures merging anywhere on the Analisi page. Where two do share a name, charts and lists spell out which is which — "Casa (Spese Fisse)" / "Casa (Spese Variabili)" — and leave unambiguous names alone
...
| Testing | Vitest | Unit testing (1533 tests) |
